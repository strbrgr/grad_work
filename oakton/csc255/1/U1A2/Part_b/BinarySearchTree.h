...
template <typename Comparable>
int BinarySearchTree<Comparable>::getRank(const std::string &wordName) const {
  BinaryNode *current = root;

  while (current != nullptr) {
    if (wordName < current->element.name) {
      current = current->left;
    } else if (current->element.name < wordName) {
...
template <typename Comparable> int BinarySearchTree<Comparable>::size() const {
  return currentSize; // Return the size
}
